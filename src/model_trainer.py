...
class FeatureEngineer:
    ...
    def create_integrated_features(self, datasets: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
        ...
        integrated_features = self._integrate_features(
            gene_features, network_features)
    ...
    def _apply_pca_to_datasets(self, datasets: Dict[str, pd.DataFrame]) -> Dict[str, pd.DataFrame]:
        ...
        for data_type, data in datasets.items():
            ...
            n_components = self.config.get(
                'pca_components', {}).get(data_type, 100)
            n_components = min(n_components, min(data.shape) - 1)

            self.logger.info(
                f"Applying PCA to {data_type} data with {n_components} components")
            ...
            self.logger.info(
                f"PCA for {data_type}: {explained_variance:.3f} variance explained")
    ...
    def _create_gene_features(self, reduced_features: Dict[str, pd.DataFrame],
        ...
        for gene in common_genes:
            ...
            for data_type, pca_data in reduced_features.items():
                ...
                gene_features = self._get_gene_features_from_pca(
                    gene, pca_data, data_type)
    ...
    def _create_network_features(self, annotations: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        pathway_features = self._create_pathway_features(
    ...
    def _create_ppi_network(self, ppi_data: pd.DataFrame) -> nx.Graph:
        ...
        self.logger.info(
            f"Created PPI network with {G.number_of_nodes()} nodes and {G.number_of_edges()} edges")
        return G
    ...
    def _calculate_network_metrics(self, network: nx.Graph) -> pd.DataFrame:
        ...
        betweenness_centrality = nx.betweenness_centrality(
            network, k=1000)  # Sample for speed
        ...
        self.logger.info(
            f"Calculated network metrics for {len(metrics_df)} nodes")
        return metrics_df
    ...
    def _create_pathway_features(self, pathway_data: pd.DataFrame) -> pd.DataFrame:
        ...
        self.logger.info(
            f"Created pathway features for {len(pathway_features)} genes")
    ...
    def _create_go_features(self, go_data: pd.DataFrame) -> pd.DataFrame:
        ...
        bp_terms = [t for t in go_terms if hash(
            t) % 3 == 0]  # Biological Process
        mf_terms = [t for t in go_terms if hash(
            t) % 3 == 1]  # Molecular Function
        cc_terms = [t for t in go_terms if hash(
            t) % 3 == 2]  # Cellular Component
        ...
        for gene in genes:
            ...
            go_features.loc[gene, 'bp_terms'] = len(
                gene_terms.intersection(bp_terms))
            go_features.loc[gene, 'mf_terms'] = len(
                gene_terms.intersection(mf_terms))
            go_features.loc[gene, 'cc_terms'] = len(
                gene_terms.intersection(cc_terms))
    ...
    def _integrate_features(self, gene_features: pd.DataFrame,
        ...
        integrated = pd.concat(
            [gene_features_aligned, network_features_aligned], axis=1)

        # Handle missing values
        integrated = integrated.fillna(0)

        self.logger.info(
            f"Integrated features for {len(integrated)} genes with {len(integrated.columns)} features")
        return integrated
    ...
    def _apply_multiview_fusion(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(views) < 2:
            self.logger.warning(
                "Not enough views for CCA, returning original features")
            return features
    ...
    def _split_features_into_views(self, features: pd.DataFrame) -> List[List[str]]:
        ...
        transcriptomics_features = [
            col for col in features.columns if 'transcriptomics' in col]
        proteomics_features = [
            col for col in features.columns if 'proteomics' in col]
        epigenomics_features = [
            col for col in features.columns if 'epigenomics' in col]
        metabolomics_features = [
            col for col in features.columns if 'metabolomics' in col]
        network_features = [col for col in features.columns if any(
            net in col for net in ['centrality', 'pathway', 'go', 'clustering'])]
    ...
    def load_labels(self) -> pd.DataFrame:
        """Load gene labels from CSV file"""
        labels_df = pd.read_csv(self.config['labels_filepath'])
        # Ensure column names are clean
        labels_df.columns = labels_df.columns.str.strip()
        self.logger.info(
            f"Loaded labels for {len(labels_df)} genes from {self.config['labels_filepath']}")
        return labels_df
...
class ModelTrainer:
    ...
    def train(self, features: dict) -> dict:
        ...
        # Filter features to match genes with available labels
        # Assuming the gene symbols are in the index of X
        available_genes = labels_df['pipe_genesymbol'].values
        common_genes = X.index.intersection(available_genes)

        if len(common_genes) == 0:
            self.logger.error(
                "No common genes found between features and labels")
            raise ValueError(
                "No common genes found between features and labels")

        # Filter X to only include genes with labels
        X_filtered = X.loc[common_genes]

        # Create labels vector matching the filtered features
        labels_dict = dict(
            zip(labels_df['pipe_genesymbol'], labels_df['is_True']))
        y = pd.Series([labels_dict[gene]
                      for gene in common_genes], index=common_genes)

        self.logger.info(f"Training on {len(common_genes)} genes with labels")
        self.logger.info(
            f"Risk genes: {y.sum()}, Non-risk genes: {len(y) - y.sum()}")

        if X_filtered.shape[0] < 1:
            self.logger.error(
                f"No samples available for training (X shape: {X_filtered.shape}). Aborting model training.")
            raise ValueError(
                f"No samples available for training. Check your data preprocessing and feature engineering steps.")
        ...
        self.logger.info(
            f"Trained RandomForest model on {X_filtered.shape[0]} samples, {X_filtered.shape[1]} features.")
